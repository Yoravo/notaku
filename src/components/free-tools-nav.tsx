...
export function FreeToolsNav() {
  ...
  return (
    <section className="mt-14 sm:mt-20 border-t border-line dark:border-slate-800 pt-12">
      <div className="text-center max-w-2xl mx-auto mb-8">
        <h2 className="text-lg sm:text-2xl font-extrabold text-ink tracking-tight">
          Peralatan Bisnis & Keuangan Gratis Lainnya
        </h2>
        <p className="text-xs sm:text-sm text-ink-soft dark:text-slate-400 mt-1">
        ...
        {tools.map((tool) => {
          ...
          return (
            <Link
              ...
              href={tool.href}
              prefetch={true}
              className="group p-4 rounded-2xl bg-white dark:bg-slate-900 border border-line dark:border-slate-800 hover:border-emerald/40 dark:hover:border-emerald-500/40 hover:shadow-md transition-all duration-200 flex items-start gap-3.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald min-h-[44px]"
            >
              <div className="p-2.5 rounded-xl bg-emerald/10 text-emerald dark:text-emerald-400 group-hover:bg-emerald group-hover:text-white transition-colors shrink-0">
                <Icon className="w-5 h-5" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  <span className="text-xs sm:text-sm font-bold text-ink group-hover:text-emerald dark:group-hover:text-emerald-400 transition-colors truncate">
                    {tool.title}
                  </span>
                  {tool.badge && (
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald/10 text-emerald dark:text-emerald-400 border border-emerald/20 shrink-0">
                      {tool.badge}
                    </span>
                  )}
                </div>
                <p className="text-[11px] sm:text-xs text-ink-soft dark:text-slate-400 line-clamp-2 leading-relaxed">
                  {tool.desc}
                </p>
              </div>
            </Link>
