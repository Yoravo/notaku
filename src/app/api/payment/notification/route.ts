...
export async function POST(request: Request) {
  try {
    ...
    if (Number(gross_amount) !== PRO_PRICE) {
      auditLog("payment.invalid_amount", { orderId: order_id, grossAmount: gross_amount });
    ...
    if (isSettlement && isFraudOk) {
      // extend from current period that exist, not from today
